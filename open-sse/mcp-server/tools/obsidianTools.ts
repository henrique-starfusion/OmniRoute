...
import { getObsidianConfigForApiKey } from "../../../src/lib/db/obsidian.ts";
import type { ObsidianClient } from "../../../src/lib/obsidian/api.ts";
...
function requireSyncToken(): string {
  const token = getSyncToken();
  if (!token) {
    throw new Error(
      "Obsidian sync token not configured. Set the dedicated OBSIDIAN_SYNC_TOKEN value."
    );
  }
  return token;
}
...
export const obsidianTools = [
  ...
  },
  {
    ...
    description:
      "Get the embedded OmniRoute sync server status, vault name, port, uptime, and last local inventory result. Requires OBSIDIAN_SYNC_TOKEN.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown) => {
      const token = requireSyncToken();
  ...
  },
  {
    ...
    description:
      "Inventory the configured local Obsidian vault. Reports local file and conflict counts without claiming external transfer. Requires OBSIDIAN_SYNC_TOKEN.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown) => {
      const token = requireSyncToken();
  ...
  },
  {
    ...
    description:
      "List unresolved local .conflict-* files. Each item includes its canonical path, conflict path, and detection time. Requires OBSIDIAN_SYNC_TOKEN.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown) => {
      const token = requireSyncToken();
  ...
  },
  {
    ...
    description:
      "Resolve a local conflict artifact. Use 'local' for the .conflict-* content, 'remote' for the canonical file, or 'keep-both' to preserve both. Requires OBSIDIAN_SYNC_TOKEN.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
      path: z
        .string()
      ...
      resolution: z
        ...
        .describe("Which version to keep: local (.conflict-*), remote (canonical), or keep-both"),
    }),
    handler: async (args: { path: string; resolution: "local" | "remote" | "keep-both" }) => {
      const token = requireSyncToken();
      const syncClient = createSyncServerClient(token);
