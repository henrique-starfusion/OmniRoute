...
export function getSyncToken(): string | null {
  return process.env.OBSIDIAN_SYNC_TOKEN?.trim() || null;
}
...
export function createSyncServerClient(syncToken: string, baseUrl?: string) {
  const resolvedBaseUrl = (
    baseUrl?.trim() ||
    process.env.OBSIDIAN_SYNC_SERVER_URL?.trim() ||
  ...
  async function request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      await res.text();
      throw new Error(`Obsidian sync server request failed (${res.status})`);
    }
